...
#ifndef T_MODULES_AUDIO_PROCESSING_AGC_MAIN_INTERFACE_GAIN_CONTROL_H_
#define T_MODULES_AUDIO_PROCESSING_AGC_MAIN_INTERFACE_GAIN_CONTROL_H_
...
typedef struct
{
    T_Word16 targetLevelDbfs;   // default 3 (-3 dBOv)
    T_Word16 compressionGaindB; // default 9 dB
    T_UWord8 limiterEnable;     // default kAgcTrue (on)
} TAgc_config_t;
 ...
 */
	/*
int TAgc_AddFarend(void* agcInst,
                        const T_Word16* inFar,
                        T_Word16 samples);
						*/
 ...
 *                          : -1 - Error
 */
int TAgc_VirtualMic(void* agcInst,
                         T_Word16* inMic,
                         T_Word16* inMic_H,
                         T_Word16 samples,
                         T_Word32 micLevelIn,
                         T_Word32* micLevelOut);
 ...
 *                          : -1 - Error
 */
int TAgc_Process(void* agcInst,
                      const T_Word16* inNear,
                      const T_Word16* inNear_H,
                      T_Word16 samples,
                      T_Word16* out,
                      T_Word16* out_H,
                      T_Word32 inMicLevel,
                      T_Word32* outMicLevel,
                      T_Word16 echo,
                      T_UWord8* saturationWarning);
 ...
 *                          : -1 - Error
 */
int TAgc_set_config(void* agcInst, TAgc_config_t config);
 ...
 *                          : -1 - Error
 */
//int TAgc_get_config(void* agcInst, TAgc_config_t* config);
 ...
 *                            -1 - Error
 */
int TAgc_Init_org(void *agcInst,
                   T_Word32 minLevel,
                   T_Word32 maxLevel,
                   T_Word16 agcMode,
                   T_UWord32 fs);
 ...
 *                            -1 - Error
 */
int TAgc_Version(T_Word8 *versionStr, T_Word16 length);

#if defined(__cplusplus)
}
#endif

#endif // T_MODULES_AUDIO_PROCESSING_AGC_MAIN_INTERFACE_GAIN_CONTROL_H_
