...
SSTParameters *SegSSLWithHead_Init_float(int MicNum, float *MicPosMM, int SampleRateHz, AFE_CODE *ReturnCode);

AFE_CODE SegSSLWithHead_Process_float(SSTParameters *st, short *MicIn, int PointNum, int *DOADegree);


AFE_CODE SegSSLWithHead_Exit_float(SSTParameters *st);

int SegWithGetOneFrame(SSTParameters *st);
	
int SegWithGetCrossSpec(SSTParameters *st, float Spec0[FFT_SIZE], float Spec1[FFT_SIZE],  OnePair *PairPt);

int SegSSLWithHead_ProcessOnePair(SSTParameters *st, OnePair *PairPt, int PairIdx);


int SegWithTrackNoise(SSTParameters *st, OnePair *PairPt, int PairIdx);

int SegWithRecordIPD(SSTParameters *st, OnePair *PairPt, int BinCountIn, float IPD, int PairIdx);

int SegSSLWithHead_SelectDirectMic(SSTParameters *st);

int SegSSLWithHead_PairSSL(SSTParameters *st, OnePair *PairPt);