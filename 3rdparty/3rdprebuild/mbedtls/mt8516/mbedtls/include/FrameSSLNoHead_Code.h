...
SSTParameters *FrameSSLNoHead_Init_float(int MicNum, float *MicPosMM, int SampleRateHz, int DelayPointNum, AFE_CODE *ReturnCode);

AFE_CODE FrameSSLNoHead_Process_float(SSTParameters *st, short *MicIn, int PointNum, int *DOADegree);

AFE_CODE FrameSSLNoHead_Exit_float(SSTParameters *st);

int FrameNoGetOneFrame(SSTParameters *st);
	
int FrameNoGetCrossSpec(float Spec0[FFT_SIZE], float Spec1[FFT_SIZE],  MicGroupPara *GroupPt, int GroupIdx, int PairIdx);

int FrameSSLNoHead_ProcessOneGroup(SSTParameters *st, MicGroupPara *CouplePt, int GroupIdx);

int FrameNoTrackNoise(SSTParameters *st, MicGroupPara *GroupPt, int GroupIdx, int PairIdx);

int FrameNoRecordIPD(SSTParameters *st, MicGroupPara *GroupPt, int BinCountIn, float IPD, int GroupIdx, int PairIdx);
