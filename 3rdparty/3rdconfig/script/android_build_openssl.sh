...
ANDROID_NDK=~/toolchain/android-ndk-r15c
OPENSSL_VERSION=1.1.1g

API_LEVEL=19

BUILD_DIR=/tmp/openssl_android_build
OUT_DIR=/home/wangyf/opt/openssl-android

BUILD_TARGETS="armeabi-v7a x86 armeabi"
# BUILD_TARGETS="armeabi armeabi-v7a x86 arm64-v8a"

#if [ ! -d openssl-${OPENSSL_VERSION} ]
#then
#    if [ ! -f openssl-${OPENSSL_VERSION}.tar.gz ]
#    then
#        wget --no-check-certificate https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz || exit 128
#    fi
#    tar xzf openssl-${OPENSSL_VERSION}.tar.gz || exit 128
#fi
#
#cd openssl-${OPENSSL_VERSION} || exit 128
cd /home/wangyf/opt/openssl-1.1.1g || exit 128
...
##### build-function #####
build_the_thing() {
    TOOLCHAIN=$ANDROID_NDK/toolchains/$TC_NAME/prebuilt/linux-x86_64
...
#### copy libraries and includes to output-directory #####
    ...
    cp -R $DESTDIR/usr/local/lib/* $OUT_DIR/lib/$build_target
done
